...
class WPCF7_ContactForm {
	...
	function mail() {
		...
		if ( $this->compose_and_send_mail( $this->mail ) ) {
			...
			foreach ( $additional_mail as $mail )
				$this->compose_and_send_mail( $mail, $pipes );
	...
	function compose_and_send_mail( $mail_template ) {
		...
		$the_pipes = $this->get_the_pipes();
		
		$body = $mail_template['body'];
		
		$myrecipient = $this->posted_data;
		$myrecipient = $myrecipient['recipient'];
		
		/*foreach ($the_pipes as $pipe)
		{
			...
			//$body = $this->get_string_between($mail_template['body'], $str, $str2);
			
			
		}
		*/
		$str = "[response-$myrecipient]";
		$str2 = "[/response-$myrecipient]";
		
		$body = $this->get_string_between($mail_template['body'], $str, $str2);
		
		if ( $use_html ) {
			$body = preg_replace_callback( $regex, $callback_html, $body );
			$body = wpautop( $body );
		} else {
	...
	function find_pipe($after)
	{		
		foreach ($this->mypipes as $pipe)
		{
			foreach ($pipe->pipes as $pip)
			{
				if ($pip->after == $after)
				{
					return $pip->before;
	...
	function mail_callback( $matches, $html = false ) {
		...
		$label_pos = strpos( $matches[1], '-label' );
		
		if ($label_pos)
		{
			$after = $this->posted_data[substr( $matches[1], 0, $label_pos )];
			
			return $this->find_pipe($after);
		}
